...
//					}
	...
	//slide toggleAccordion v2
	$.fn.toggleAccordionV2 = function(options) {
		var settings = $.extend({
			wrapClass:".js_wrap",
			clickShowClass:".js_click",
			showClass:".js_show",
			itemClickClass:"",//".item ul li",
			hideClass:"",//".bt_close, ul li,....",
			fade:false,
			multiShow:false,
			layer:false,
			duration:400
		...
		try{
			// Iterate over each .js_wrap3 element
			$(settings.wrapClass).each(function() {
				var $this = $(this);
				var clickShowClass = $this.find(settings.clickShowClass);
				var showClass = $this.find(settings.showClass);
				var hideClass = $this.find(settings.hideClass);
				var itemClickClass = $this.find(settings.itemClickClass);

				// Add click event handler to the trigger element
				clickShowClass.on('click', function(e){
					e.preventDefault();
					var $clickedShowClass = $(this).siblings(settings.showClass);
					if (settings.multiShow) {
						$(this).toggleClass('on'); // Toggle the class .on on the trigger element
						if (settings.fade) {
							$clickedShowClass.stop(true, true).fadeToggle(settings.duration); // Toggle the clicked content associated with the trigger element
						} else {
							$clickedShowClass.stop(true, true).slideToggle(settings.duration); // Toggle the clicked content associated with the trigger element
						}
					} else {
						if($(this).hasClass('on')) {
							// If already open, close only the clicked one
							if (settings.fade) {
								$clickedShowClass.stop(true, true).fadeOut(settings.duration); // Toggle the clicked content associated with the trigger element
							} else {
								$clickedShowClass.stop(true, true).slideUp(settings.duration); // Toggle the clicked content associated with the trigger element
							}
						$(this).removeClass('on');
						} else {
							// Close all other visible show elements and open the clicked one
							var $visibleShowClasses = $(settings.showClass + ':visible').not($clickedShowClass);
							if (settings.fade) {
								$visibleShowClasses.stop(true, true).fadeOut(settings.duration);
								$(settings.clickShowClass).removeClass('on'); // Remove 'on' class from all trigger elements
								$(this).addClass('on'); // Add 'on' class to the clicked trigger element
								$clickedShowClass.stop(true, true).fadeIn(settings.duration); // Toggle the clicked content associated with the trigger element
							} else {
								$visibleShowClasses.stop(true, true).slideUp(settings.duration);
								$(settings.clickShowClass).removeClass('on'); // Remove 'on' class from all trigger elements
								$(this).addClass('on'); // Add 'on' class to the clicked trigger element
								$clickedShowClass.stop(true, true).slideDown(settings.duration); // Toggle the clicked content associated with the trigger element
							}
				...
				// Add click event handler to the itemClickClass elements
				itemClickClass.on('click', function(e){
					e.preventDefault();
					$(this).siblings().removeClass('on'); // Remove 'on' class from siblings
					$(this).addClass('on'); // Add 'on' class to the clicked element
				});

				// Hide the content when clicking on the hideClass elements
				hideClass.on('click', function(e){
					e.preventDefault();
					if (settings.fade) {
						showClass.fadeOut(settings.duration);
					} else {
						showClass.slideUp(settings.duration);
					}
					clickShowClass.removeClass('on'); // Remove class .on from the trigger element when the content is hidden
				});

				// Hide the content when clicking outside of it
				$(document).on('click', function(e){
					var container = $(settings.showClass);
					var triggerElement = $(settings.clickShowClass);
					var wrapContainer = $(settings.wrapClass);
					// If the clicked element is not within the container or its trigger element
					if(settings.layer){
						if (!wrapContainer.is(e.target) && wrapContainer.has(e.target).length === 0 && !$(e.target).is(settings.clickShowClass)) {
							if (settings.fade) {
								container.fadeOut(settings.duration);
							} else {
								container.slideUp(settings.duration);
							}
							triggerElement.removeClass('on'); // Remove class .on from the trigger element when the content is hidden
						}
					}
				});
			});
	...
	$.fn.selectTextV2 = function(options){
		try {
			var defaults = {wrapClass: ".x", clickOn: ".x", setTextToClass: ".x"};
			options = $.extend(defaults, options);
			$(this).on('click', options.clickOn, function(){
				var text = $(this).text(); // Get the text of the clicked element
				var $targetElement = $(this).closest(options.wrapClass).find(options.setTextToClass); // Find the target element to set the text
				$targetElement.text(text); // Set the text of the target element
			});
		} catch(err) {
			return;
		}
