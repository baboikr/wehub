...
/* (start)20240619 */
	$(window).scroll(function() {
		...
		$('.header,.footer_grp').css({"left":-scrollLeft});
		menuEvent();
	});
/* //(end)20240619 */
...
/* (start)20240619 */
	function hasVerticalScroll() {
		...
		$("body").append('<div class="checkSize"></div>');
		// Add CSS properties to the checkScreen div
		$(".checkSize").css({"position": "fixed","bottom": "0","left": "0","height": "0"});
		let space = null;
		if($("body").hasClass("fixed")){space =  $(".checkSize").offset().top - ($('.wrap').height()+footerH);
		}else{space =  $(".checkSize").offset().top - ($('.wrap').height());}
		//alert(resH);
		// Check if the document height is greater than or equal to the offset top of the checkScreen div
		var hasScroll = space > 1;
		$(".checkSize").remove();
		return hasScroll;
	}
	...
	updateBodyClass();
	// Check on resize
	$(window).on('resize', updateBodyClass);
/* //(end)20240619 */
});
